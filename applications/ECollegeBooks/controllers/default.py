...
def add_reply():
    ...
    form = SQLFORM(db.reply).factory(
        Field('reply_content', 'text'),
    )

    if form.process().accepted:
        db.reply.insert(
            reply_content = form.vars.reply_content,
            post_id = post_id
        )
...
def edit_reply():
    ...
    if reply.reply_author != auth.user.email:
        logging.info("Attempt to edit someone's post by : %r" % auth.user.email)
        redirect(URL('default', 'index'))
