    ...
    if auth.user is None:
        ...
        rows = db().select(db.post.ALL, db.user_like.ALL, db.user_star.ALL,
                            left=[
                                ...
                                db.user_hate.on((db.user_hate.post_id == db.post.id) & (db.user_hate.user_email == auth.user.email)),
                            ],
        ...
        for row in rows:
            results.append(dict(
                ...
                hate = False if row.user_like.id is None else True,
            ))
...
def set_hate():
    post_id = int(request.vars.post_id)
    hate_status = request.vars.hate.lower().startswith('t');
    if hate_status:
        db.user_hate.update_or_insert(
            (db.user_hate.post_id == post_id) & (db.user_hate.user_email == auth.user.email),
            post_id = post_id,
            user_email = auth.user.email
        )
    else:
        db((db.user_hate.post_id == post_id) & (db.user_hate.user_email == auth.user.email)).delete()
    return "ok"


def get_haters():
    post_id = int(request.vars.post_id)
    rows = db(db.user_hate.post_id == post_id).select(db.user_hate.user_email)
    haters_set = set([r.user_email for r in rows])
    if auth.user:
        haters_set -= {auth.user.email}
    haters_list = list(haters_set)
    haters_list.sort()
    return response.json(dict(haters=haters_list))
